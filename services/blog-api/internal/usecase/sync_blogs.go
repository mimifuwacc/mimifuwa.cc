...
import (
	...
	"github.com/mimifuwacc/blog-api/internal/repository"
)
...
func (uc *SyncBlogsUseCase) Execute(ctx context.Context, all bool, remote bool) (*entity.SyncResult, error) {
	...
	// Filter by changed files if not --all
	var filesToSync []string
	if all {
		filesToSync = markdownFiles
	} else {
		changedFiles, err := uc.git.GetChangedFiles(ctx, "")
		if err != nil {
			return nil, fmt.Errorf("failed to get changed files: %w", err)
		}

		for _, file := range markdownFiles {
			relPath, _ := filepath.Rel(uc.blogsDir, file)
			if uc.contains(changedFiles, relPath) {
...
func (uc *SyncBlogsUseCase) syncFile(ctx context.Context, filePath string, result *entity.SyncResult) error {
	...
	// Note: We need to query for this since the upsert doesn't return the ID
	// In a real implementation, you'd add a method to get ID by slug

	// Handle tags
	if len(post.Tags) > 0 {
		// First, we need to get the blog post ID
		// This is a simplification - in production, you'd have a proper method
		// For now, we'll skip tag handling in this basic implementation
		fmt.Printf("  Tags: %v\n", post.Tags)
	}
